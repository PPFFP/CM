...
async def skip(c: Client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            ...
            await c.send_message(chat_id, "⥁⁞ مسح قوائم الانتظار ** \n \n ** ⥁⁞  مغادرة المستخدم الآلي للدردشة الصوتية **  ")
        else:
            ...
            await c.send_photo(
                ...
                caption=f"**⥁⁞ تم التخطي الئ المسار الت*\n **⥁⁞ الاسم:** [{op[0]}]({op[1]})\n **⥁⁞الحالة:** `شغال`\n **⥁⁞ صاحب الطلب :** {m.from_user.mention()}}",
            )
    else:
        skip = m.text.split(None, 1)[1]
        OP = "⥁⁞ عـزيزي :** {m.from_user.mention()} \n **⥁⁞  تمت إزالة الأغنية من قائمة الانتظار:**"
        if chat_id in QUEUE:
...
@Client.on_message(
    command(["stop", f"stop@{BOT_USERNAME}", "end", f"نهاء", "انهاء", "ايقاف"])
    & other_filters
)
@authorized_users_only
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("⥁⁞ عـزيزي :** {m.from_user.mention()} \n **⥁⁞  تم ايقاف التشغيل **")
...
@Client.on_message(
    command(["pause", f"pause@{BOT_USERNAME}", "vpause", "توقف"]) & other_filters
)
@authorized_users_only
async def pause(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                "⥁⁞ عـزيزي :** {m.from_user.mention()} \n **⥁⁞  تم ايقاف التشغيل مؤقتا **"
            )
        except Exception as e:
            await m.reply(f"🚫 **خطأ:**\n\n`{e}`")
    else:
...
@Client.on_message(
    command(["mute", f"mute@{BOT_USERNAME}", "vmute", "كتم"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["unmute", f"unmute@{BOT_USERNAME}", "vunmute", "الغاء كتم"]) & other_filters
)
@authorized_users_only
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.leave_group_call(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("⥁⁞ عـزيزي :** {m.from_user.mention()} \n **⥁⁞  تم ايقاف التشغيل **", reply_markup=close_mark)
        except Exception as e:
            await query.edit_message_text(f"🚫 **خطأ:**\n\n`{e}`", reply_markup=close_mark)
    else:
        await query.answer("**⥁⁞  قائمة التشغيل فارغه**", show_alert=True)
...
async def cbmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text(
                "⥁⁞ عـزيزي :** {m.from_user.mention()} \n **⥁⁞  تم ڪتم البوت", reply_markup=back_mark
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **خطأ:**\n\n`{e}`", reply_markup=close_mark)
    else:
...
async def cbunmute(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text(
                "⥁⁞ عـزيزي :** {m.from_user.mention()} \n **⥁⁞  تم الغاء ڪتم البوت", reply_markup=back_mark
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **خطأ:**\n\n`{e}`", reply_markup=close_mark)
    else:
...
async def change_volume(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            await m.reply(
                f"⥁⁞ عـزيزي :** {m.from_user.mention()} \n **تم ضبط الصوت على** `{range}`%"
            )
        except Exception as e:
            await m.reply(f"🚫 **خطأ:**\n\n`{e}`")
    else:
